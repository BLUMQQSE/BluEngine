...
void DummyAI::take_damage(unsigned damage, EnumFlag damage_type, EnumFlag damage_target)
{
	health -= damage;
	...
	std::weak_ptr<TextUITag> t = UITagSystem::Instance()->create_text_tag(game_object->get_center() + Vector2f(0, -32), "-" + std::to_string(damage), 16, 3);
