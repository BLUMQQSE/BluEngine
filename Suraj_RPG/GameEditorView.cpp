...
{
	heir_panel = new GUI::Panel(0, 0, Renderer::get_window_size().x / 6, Renderer::get_window_size().y);
	inspec_panel = new GUI::Panel(Renderer::get_window_size().x - Renderer::get_window_size().x / 6, 0,
		Renderer::get_window_size().x / 6, Renderer::get_window_size().y);
}
...
void GameEditorView::toggle_editor(EditorPanel panel_to_toggle)
{
	...
	{
	case GameEditorView::EditorPanel::ALL:
		...
		break;
	case GameEditorView::EditorPanel::HEIRARCHY:
...
void GameEditorView::create_heir_panel()
{
	...
	std::vector<GameObject*> objs = SceneManager::get_objects_in_scene();
	...
	for (std::size_t i = 0; i < objs.size(); i++)
	{
		...
		GUI::Button* new_button = new GUI::Button(x, 40 * i, 100.f, 40.f, &ResourceManager::get_font("calibri-regular.ttf"), 
...
void GameEditorView::update_heir_panel()
{
	...
	if (gameobject_held == true && core::Input::get_mouse_up(core::Input::Mouse::LEFT))
	{
	...
	if (gameobject_held == true && !core::Input::get_mouse(core::Input::Mouse::LEFT))
	{
		gameobject_held = false;
		return;
	}

	if (Input::get_mouse_down(core::Input::Mouse::LEFT))
	{
		for (auto& ois : objects_in_scene_map)
		{
			if (ois.first->mouse_in_bounds())
	...
	, std::vector<Editor::SerializedVar> vars)
{
	...
	items_in_panel.push_back(std::make_pair("title", new GUI::Label(0, 0, 16, ResourceManager::get_font("calibri-regular.ttf"), component_name)));
	height += 25;
	...
	for (std::size_t i = 0; i < vars.size(); i++)
	{
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			items_in_panel.push_back(std::make_pair(vars[i].name, new GUI::InputBox(150, height, 40, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			items_in_panel.push_back(std::make_pair(vars[i].name, new GUI::DropDownList(150, height, 80, 15, ResourceManager::get_font("calibri-regular.ttf"),
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			items_in_panel.push_back(std::make_pair("x_" + vars[i].name, new GUI::InputBox(110, height, 40, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
			items_in_panel.push_back(std::make_pair("y_" + vars[i].name, new GUI::InputBox(170, height, 40, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			items_in_panel.push_back(std::make_pair("ix_" + vars[i].name, new GUI::InputBox(110, height, 40, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
			items_in_panel.push_back(std::make_pair("iy_" + vars[i].name, new GUI::InputBox(170, height, 40, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
		...
		case Editor::VarType::FloatConvex:
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, 
				ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			...
			for (std::size_t i = 0; i < polygon.getPointCount(); i++)
			{
				items_in_panel.push_back(std::make_pair(std::to_string(i) + "_x_" + vars[i].name, new GUI::InputBox(110, height, 40, 12, 12,
					ResourceManager::get_font("calibri-regular.ttf"))));
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 16, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
		...
		{
			items_in_panel.push_back(std::make_pair("l_" + vars[i].name, new GUI::Label(0, height, 12, ResourceManager::get_font("calibri-regular.ttf"), vars[i].name)));
			items_in_panel.push_back(std::make_pair(vars[i].name, new GUI::InputBox(70, height, 140, 12, 12, ResourceManager::get_font("calibri-regular.ttf"))));
